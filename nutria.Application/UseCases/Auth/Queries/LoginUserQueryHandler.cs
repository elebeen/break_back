...
internal sealed record LoginUserQueryHandler : IRequestHandler<LoginUserCommand, string>
{
    ...
    public async Task<string> Handle(LoginUserCommand request, CancellationToken cancellationToken)
    {
        ...
        if (existingUser == null || !BCrypt.Net.BCrypt.Verify(request.UserLogin.Password, existingUser.PasswordHash))
        {
            return "Credentials do not match";
        }
        
        var token = _jwtService.GenerateJwtToken(existingUser.Id.ToString(), existingUser.FullName);
        return token;
    }
}